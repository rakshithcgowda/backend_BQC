...
class Database {
    ...
    async createUser(userData) {
        return new Promise((resolve, reject) => {
            ...
            this.db.run('INSERT INTO users (username, password, email, full_name) VALUES (?, ?, ?, ?)', [username, password, email, fullName], function (err) {
                if (err) {
                    reject(err);
                }
                else {
    ...
    async getUserById(id) {
        return new Promise((resolve, reject) => {
            this.db.get('SELECT id, username, email, full_name, created_at FROM users WHERE id = ?', [id], (err, row) => {
                if (err) {
                    reject(err);
                }
                else {
    ...
    async saveBQCData(userId, bqcData) {
        return new Promise((resolve, reject) => {
            ...
            this.db.get('SELECT id FROM bqc_data WHERE user_id = ? AND ref_number = ?', [userId, bqcData.refNumber], (err, existingRecord) => {
                if (err) {
                    reject(err);
                    return;
                }
    ...
    async getBQCData(userId, id) {
        return new Promise((resolve, reject) => {
            this.db.get('SELECT * FROM bqc_data WHERE id = ? AND user_id = ?', [id, userId], (err, row) => {
                if (err) {
                    reject(err);
                }
                else {
    ...
    async listBQCData(userId) {
        return new Promise((resolve, reject) => {
            this.db.all('SELECT id, ref_number, tender_description, created_at FROM bqc_data WHERE user_id = ? ORDER BY created_at DESC', [userId], (err, rows) => {
                if (err) {
                    reject(err);
                }
                else {
    ...
    async deleteBQCData(userId, id) {
        return new Promise((resolve, reject) => {
            this.db.run('DELETE FROM bqc_data WHERE id = ? AND user_id = ?', [id, userId], (err) => {
                if (err) {
                    reject(err);
                }
                else {
